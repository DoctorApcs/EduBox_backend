...
@assistant_router.get(
    "/{assistant_id}/conversations", response_model=List[ConversationResponse]
)
async def get_assistant_conversations(
    ...
    conversations = assistant_service.get_assistant_conversations(
...
@assistant_router.post(
    "/{assistant_id}/conversations", response_model=ConversationResponse
)
...
@assistant_router.post(
    "/{assistant_id}/conversations/{conversation_id}/chat", response_model=ChatResponse
)
...
async def stream_chat_with_assistant(
    ...
    def event_generator() -> Generator[str, None, None]:
        for chunk in assistant_service.stream_chat_with_assistant(
            conversation_id, current_user_id, message
        ):
            yield chunk
...
@assistant_router.get(
    "/{assistant_id}/conversations/{conversation_id}/history",
...
async def websocket_conversation(
    ...
    try:
        while True:
            ...
            await ws_manager.send_status(websocket, "message_received")
            ...
            try:
                ...
                await ws_manager.send_error(websocket, error_message)
                await ws_manager.send_end_message(
                    websocket,
                    MediaType.TEXT,
                    EndStatus.ERROR,
